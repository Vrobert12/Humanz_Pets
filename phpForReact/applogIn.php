...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($data["userId"]) && isset($data["token"])) {
        ...
        $stmt = $pdo->prepare("SELECT session_token FROM user WHERE userId = :userId");
        ...
        if ($user && $user["session_token"] === $token) {
            ...
            $userLangStmt = $pdo->prepare("SELECT usedLanguage FROM user WHERE userId = :userId");
    ...
    $stmt = $pdo->prepare("SELECT userId, userPassword, usedLanguage FROM user WHERE userMail = :email");
